...
class TyranoScriptParser {
	...
	parse(script: string): Node[] {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			let column = line.length - line.trim().length;
	...
	private parseTag(
		...
		while (currentColumn < line.length) {
			if (!isQuoted && (line[currentColumn] === '"' || line[currentColumn] === "'")) {
				quote = line[currentColumn];
				isQuoted = true;
			} else if (isQuoted && line[currentColumn] === quote) {
				isQuoted = false;
			} else if (!isQuoted && line[currentColumn] === ']') {
				break;
			}
		...
		console.log(tagContent.slice(1, -1));
